...
def upload_xml(xml_string):
    ...
    filename = "interview_voicebot.xml"
    data = {
        "description": "Data Science Interview VoiceBot XML",
        "public": False,
        "files": {
            filename: {
                "content": xml_string
...
def create_application(xml_url):
    ...
    response = client.applications.create(
        app_name='data-science-interview-bot',
        answer_url=xml_url,
        answer_method='GET'
    )
...
def update_phone_number(phoneNumber, appId):
    client = plivo.RestClient(CONFIG['auth_id'], CONFIG['auth_token'])
    number = client.numbers.get(number=phoneNumber)
    response = number.update(app_id=appId)
